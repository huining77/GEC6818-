	...
	while (1)
	{
		...
		switch (dir)
		{
			...
			case TouchDown:
                ...
                draw_bmp(0,0,"photo_2.bmp");
				// DisplayBmpPicVideo("./photo_2.bmp",1);
			break;
			case TouchLeft:
                draw_bmp(0,0,"photo_3.bmp");
				// DisplayBmpPicVideo("./photo_3.bmp",2);
			break;
			case TouchRight:
                draw_bmp(0,0,"photo_4.bmp");
...
// */
// void draw_bmp(int x0,int y0,char *pathname)
// {
...
//     int pa_size = size - 0x36; //像素数组大小 = 文件大小 - 前三个部分的大小
//     char buf[pa_size];
//     lseek(fd,0x36,SEEK_SET);
//     read(fd,buf,pa_size); //读取像素数组
...
//     for(int k=0;k<h;k++) //有多少行
//     {
//         for(int j=0;j<w;j++) //显示每一行的像素点
//         {
//             b = buf[i++];
//             g = buf[i++];
//             r = buf[i++];
...
//             draw_point( x0+j, y0+h-1-k, color);
//         }
//         i+=fill; //跳过填充字节
//     }
// }
...
void draw_point(int x, int y, int color) {
    if (x >= 0 && x < 800 && y >= 0 && y < 480) { // x, y are within bounds
        *(plcd + y * 800 + x) = color;
        // printf("%d\n", count++);
    }
}
...
/*
    draw_bmp: Image display function
    (x0, y0): The coordinates where the image will be displayed
    pathname: The name of the image file
*/
void draw_bmp(int x0, int y0, char *pathname) {
    int fd = open(pathname, O_RDONLY);
    if (fd == -1) {
    ...
    // Center the image if it fits within the display dimensions
    if (w < 800) {
        x0 = (800 - w) / 2;
    }
    if (h < 480) {
    ...
    // Read the file size to calculate the size of the pixel array
    int size;
    lseek(fd, 0x02, SEEK_SET);
    read(fd, &size, 4);

    int pa_size = size - 0x36; // Pixel array size = file size - first three sections' size
    char buf[pa_size];
    lseek(fd, 0x36, SEEK_SET);
    read(fd, buf, pa_size); // Read the pixel array

    char a = 0, r, g, b;
    int i = 0;

    int fill = 0; // Calculate padding bytes
    if (w * (d / 8) % 4 != 0) {
        fill = 4 - (w * (d / 8) % 4);
    }

    for (int k = 0; k < h; k++) { // For each row
        for (int j = 0; j < w; j++) { // Display each pixel in the row
            b = buf[i++];
            g = buf[i++];
            r = buf[i++];
            if (d == 32) {
